...
Basic simulations of a GLDS model system fit to experimental data show the advantages of bidirectional control and of MPC.
...
##zref = [i < 3000 ? 0.1 : 0.15 for i in 1:(pred*sample)] #trial firing rate reference
zref = .1 .+ [0.08*sin(i/(3*pred*sample) * 2*pi) for i in 1:(3*pred*sample)]; #trial firing rate reference
#zref = [0.2 for i in 1:(pred*sample)] #trial firing rate reference
```
...
@time lqr1res = lqr(T, zeros(4), zref, 1);
@time lqr2res = lqr(T, zeros(4), zref, 2);
...
function mpc(steps, x0, zref; nu=1, u_clamp=nothing, sample=250)
    ...
    @variables(neuron, begin
        ...
        0 ≤ u[1:nu, 1:(ctr+1)], (start = 0)
        yD[i=1:1, t=1:(pred*sample)], (start = 0)
    ...
    @expressions(
        ...
        begin
            ...
            y_error[t=1:(pred*sample)], y[:, t] - yD[:, t]
            y_cost[t=1:(pred*sample)], y_error'[t]*y_error[t]
    ...
    fix.(yD, yDall[1:length(yD)], force=true)
    # for i in 1:(length(yD))
    #     fix.(yD[:, i], yDall[i][1]; force=true)
    # end

    if (length(yD)) < (pred*sample)
        for i in (length(yD)+1):(pred*sample)
            fix.(yD[:, i], yD[end][1]; force=true)
        end
    end
    ...
    for t in 1:steps
        ...
        if length(yDall) - sample*t >= sample
            for j in (1 + sample*t):length(yD)
                fix.(yD[:, (j-sample*t)], yDall[j][1]; force=true)
            end
            if (length(yDall) - sample*t) < (pred*sample)
                for i in (length(yDall)-sample*t + 1):(pred*sample)
                    fix.(yD[:, i], yDall[end][1]; force=true)
                end
            end
        end
...
steps = T ÷ sample
@time mpc2res = mpc(steps, zeros(4), zref, nu=2, u_clamp=nothing, sample=sample);
```
```{julia}
@time mpc1res = mpc(steps, zeros(4), zref, nu=1, u_clamp=nothing, sample=sample);
```


```{julia}
function plotctrl(zs, us; plotargs...)
    nu = size(us, 1)
    last = zref[end][1]
    if (length(zref)) < (steps*sample)
        for i in (length(zref)):(steps*sample - 1)
            append!(zref, last)
        end
    end
    ...
    time = (1:(steps*sample)) ./ 1000
    zs_plot = plot(time, [zref[1:(sample*steps)] zs'], label=["reference" "firing rate"],
    ...
    u_plot = plot(time, us', label="u", color=ucolor, lw=2, xlabel="time (s)")

    return plot(zs_plot, u_plot; layout=(2, 1), link=:x, plotargs...)
end

lqr1plot = plotctrl(lqr1res...)
lqr2plot = plotctrl(lqr2res...)
mpc1plot = plotctrl(mpc1res...)
mpc2plot = plotctrl(mpc2res...)
...
While MPC will be considerably slower than LQR, it should be fast enough for control of neural phenomena on the timescale of ~10 milliseconds.
...
One is that some variations in the control scheme can help balance speed and performance, such as letting the control horizon be shorter than the prediction horizon, shrinking the optimization problem.
